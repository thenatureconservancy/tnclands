...
def do_CreateFileGDB(targetGDBName, sourceGDBName):
    ...
    domainTableList = [(os.path.join(sourceGDBName, 'Domain_tnc_COUNTRY'), 'tnc_COUNTRY', 'Standardized country name'),
                       ...
                       (os.path.join(sourceGDBName, 'Domain_tnc_PROTMECH'), 'Domain_tnc_PROTMECH', 'The type of protection in place'),
...
def do_InsertExternalWebTNCLandsRecords(targetGDBTNCLands, sourceGDBTNCLands):
    ...
    for sRow in sRows:
        ...
        #if sRow.PROTMECH   == 'Fee Ownership' :                iRow.WEBSYM = 'Fee Ownership'
        #elif sRow.PROTMECH == 'Conservation Easement':         iRow.WEBSYM = 'Conservation Easement'
        #elif sRow.PROTMECH == 'Dead Restrictions':             iRow.WEBSYM = 'Deed Restrictions'
        #elif sRow.PROTMECH == 'Access Right of Way':           iRow.WEBSYM = 'Access Right of Way'
        #elif sRow.PROTMECH == 'Right of Way Tract' :           iRow.WEBSYM = 'Right of Way Tract'
...
def do_InsertExternalWebTransferredTNCLandsRecords(targetGDBTNCLands, sourceGDBTNCLands):
    ...
    try:
        sRows = arcpy.SearchCursor(sourceGDBTNCLands,  '("STATE" IS NOT NULL) AND ("SENSDATA" = \'NO\' OR "SENSDATA" IS NULL )')
    except arcpy.ExecuteError:
        print arcpy.GetMessages(2)
        return [False, 'ArcGIS error creating search cursor']
    except Exception as e:
    ...
    for sRow in sRows:
        ...
        try:
            ...
            print arcpy.GetMessages(2)
            return [False, 'ArcGIS error inserting record TR_ID_TEXT: {0}'.format(str(sRow.TR_IFMS_ID).replace('.0', ''))]
        except Exception as e:
            print e.args[0]
            return [False, 'System error inserting record TR_ID_TEXT: {0}'.format(str(sRow.TR_IFMS_ID).replace('.0', ''))]
...
def do_InsertExternalDownloadTNCLandsRecords(targetGDBTNCLands, sourceGDBTNCLands):
    ...
    for sRow in sRows:
        ...
        iRow.PROTMECH = sRow.PROTMECH
        iRow.SENSDATA = sRow.SENSDATA
        iRow.TR_IFMS_ID = str(sRow.TR_IFMS_ID).replace('.0', '')
        iRow.MA_IFMS_ID = str(sRow.MA_IFMS_ID).replace('.0', '')
        iRow.CLS_ACRES = sRow.CLS_ACRES
        iRow.CLSTRANSDA = sRow.CLSTRANSDA
        iRow.Shape = sRow.Shape
...
def do_InsertInternalWebTNCLandsRecords(targetGDBTNCLands, sourceGDBTNCLands):
    ...
    try:
        sRows = arcpy.SearchCursor(sourceGDBTNCLands, '"STATE" IS NOT NULL')
    except arcpy.ExecuteError:
        print arcpy.GetMessages(2)
        return [False, 'ArcGIS error creating search cursor']
    except Exception as e:
