...
chapter = 'WIFO'
#source and target file geodatabases
sourceGDB = 'D:/jplatt/projects/TNC_Lands/archive/Data_from_Dan_Mahoney/WI/TNC_Lands_WI.gdb'
targetGDB = 'D:/jplatt/projects/TNC_Lands/data/TNC_Lands_Working.gdb'
#the name of the source TNC Lands and TNC Transfers and Assists feature classes
tncLands_WGS84 = 'TNC_Lands'
tncTrnAst_WGS84 = 'TNC_TransfersAndAssists'
#feature class names for the projected versions
tncLands_WMAS = f'TNC_Lands_WMAS_{chapter}'
tncTrnAst_WMAS = f'TNC_TransfersAndAssists_WMAS_{chapter}'
#feature class name for the combined datasets
tncLndTrnAst_WMAS = f'TNC_Lands_Transfers_Assists_WMAS_{chapter}'
#local workspace name
workspace = 'D:/jplatt/projects/TNC_Lands/data/TNC_Lands_Working.gdb'
...
#check for source gdb
if arcpy.Exists(targetGDB):
    pass
else:
    try:
        arcpy.CreateFileGDB_management(os.path.dirname(targetGDB), os.path.basename(targetGDB))
    except:
...
try:
    arcpy.management.Project(in_dataset = os.path.join(sourceGDB, tncLands_WGS84),
                             out_dataset = os.path.join(targetGDB, tncLands_WMAS),
...
try:
    arcpy.management.CheckGeometry(in_features = os.path.join(targetGDB, tncLands_WMAS),
...
#repair geometry errors if necessary
if int(arcpy.management.GetCount('tncLands_WMAS_check_geometry')[0]) > 0:
    print(f'Repairing geometry errors of {tncLands_WMAS}')
    try:
        arcpy.management.RepairGeometry(in_features=os.path.join(targetGDB, tncLands_WMAS))
    except arcpy.ExecuteError:
        print(arcpy.GetMessages(2))
    except Exception as e:
        print(e.args[0])
...
try:
        arcpy.management.Project(in_dataset = os.path.join(sourceGDB, tncTrnAst_WGS84),
                                 out_dataset = os.path.join(targetGDB, tncTrnAst_WMAS),
                                 out_coor_system = srWMAS)
except arcpy.ExecuteError:
    print(arcpy.GetMessages(2))
except Exception as e:
...
try:
    arcpy.management.CheckGeometry(in_features =os.path.join(targetGDB, tncTrnAst_WMAS),
...
if int(arcpy.management.GetCount('tncTransfersAndAssists_WMAS_check_geometry')[0]) > 0:
    ...
    try:
        arcpy.management.RepairGeometry(in_features=os.path.join(targetGDB, tncTrnAst_WMAS))
    except arcpy.ExecuteError:
        print(arcpy.GetMessages(2))
    except Exception as e:
        print(e.args[0])
...
try:
    arcpy.management.CalculateGeometryAttributes(in_features = tncLndTrnAst_WMAS, 
